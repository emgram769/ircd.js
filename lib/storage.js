...
function ChannelDatabase(server, historyCallback) {
  this.server = server;
  this.registered = {};
...
ChannelDatabase.prototype = {
  ...
  join: function(user, channelName, key) {
    ...
    this.historyCallback(channel.name, function(name, message) {
      user.send(':'+name+'!Anonymous@livechan.net', 'PRIVMSG', channel.name,':'+ message);
    });
