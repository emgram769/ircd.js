...
var net = require('net'),
    ...
    winston = {info:function(){}, log:function(){}, error:function(){}},//require('winston'),
...
function Server(historyCallback, onPrivMessage) {
  this.history = new History(this);
  this.users = new UserDatabase(this);
  historyCallback = historyCallback ? historyCallback : function(chatName, send) {
  send('anna', 'Welcome!');
  return;
};
  this.channels = new ChannelDatabase(this, historyCallback);
  this.config = null;
  this.onPrivMessage = onPrivMessage ? onPrivMessage : function(){};
...
Server.prototype = {
  ...
  broadcast: function(channel, message, name) {
    if (!(channel in this.channels.registered)) return;
    this.channels.registered[channel].users.forEach(function(channelUser) {
      name = name ? name : 'Anonymous';
      channelUser.send(':'+name+'!Anonymous@livechan.net', 'PRIVMSG', channel, ':' + message);
    });
  }
};
